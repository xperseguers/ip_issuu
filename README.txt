This is just a simple Extension that lets you embed issuu-flip pdfs into you TYPO3-Site

Warning:
As of release 0.2.0 this Extension uses Extbase and Fluid as Frameworks. So make sure you are using any TYPO3 Version
from Version 4.4 and above. This version _may_ break the current configuration.
Simply add the Code new in the backend and everything will be fine again

If you find any errors or want to contribute anything feel free to add a report on the project Website at
http://forge.typo3.org